        ...
        const livedChannels = await db.Video_List.findAll({
            where: {
                [Op.and]: [
                    ...
                    { published_at: {
                            [Op.between]: [utcDate.clone().subtract(48, 'hour'), utcDate.clone().add(24, 'hour')] } }
                ]
            },
            order: [
                ['status', 'ASC'],
...
async function fetchLiveInfo(videoID) {
    ...
    try {
        ...
        db.Live_List.destroy({ where: { yt_video_id: videoID } })
        db.Video_List.destroy({ where: { yt_video_id: videoID } })
        return []
    }
}
    ...
    try {
        ...
        livedChannels.map(async(livedChannel) => {
            ...
            if (liveData.status == 'past' || liveData.status == 'missing') {
                ...
                db.Live_List.destroy({ where: { yt_video_id: liveData.id } }).then((dbResult) => {
                    logResults[liveData.yt_video_id] = dbResult
                })
                db.Video_List.update(
                        data, { where: { yt_video_id: liveData.id } }
                    ).then((dbResult) => {
                        logResults[liveData.yt_video_id] = dbResult
                    })
                    .catch((err) => {
